...
- [x] [Review][Patch][Critical] `/onboarding` was `beta-readonly` in runtime scope, disagreeing with truth table. **Fixed (sixth-pass 2026-04-13):** `betaRouteScope.ts:39` — `'/onboarding': 'beta-live'` in the `// beta-live routes` section.
- [x] [Review][Patch][Critical] Beta scope tests asserted "four beta-live routes" excluding `/onboarding`. **Fixed (sixth-pass 2026-04-13):** `betaRouteScope.test.ts:105` — "contains all five beta-live routes"; `/onboarding` verified in both `getBetaScope` and `BETA_SCOPE map` tests.
- [x] [Review][Patch][Critical] Onboarding navigated to `/my-stable` (beta-hidden). **Fixed (sixth-pass/Task 10 2026-04-13):** `OnboardingPage.tsx:365` — `navigate('/stable', { replace: true })`. JSDoc header updated.
- [x] [Review][Patch][Critical] `onError` faked success: cleared storage, showed success toast, navigated away. **Fixed (sixth-pass/Task 10 2026-04-13):** `OnboardingPage.tsx:367-369` — `onError` shows error toast only; no navigate, no storage clear.
- [x] [Review][Patch][Critical] `OnboardingGuard` trusted `localStorage.equoria-onboarding-done`. **Fixed (sixth-pass 2026-04-13):** `OnboardingGuard.tsx` — localStorage removed; comment: "Server state is the sole source of truth."
- [x] [Review][Patch][High] Skip button bypassed horse selection in beta. **Fixed (sixth-pass 2026-04-13):** `OnboardingPage.tsx:491` — `{currentStep === 0 && !isBetaMode && ...}`; in non-beta mode advances to step 1 (not completion).
- [x] [Review][Patch][High] Truth table stale re: `horsesApi.create does not exist`. **Fixed (Task 10 + sixth-pass 2026-04-13):** `horsesApi.create` implemented in `api-client.ts`; `OnboardingPage` calls it first then `authApi.advanceOnboarding`; truth table `/onboarding` row updated with correct Required APIs.
- [ ] [Review][Patch][High] Registration starter-horse creation is non-fatal (catch swallows failure at `authController.mjs:140`). **Mitigated by Task 10** (OnboardingPage now calls `horsesApi.create` independently; `onError` blocks completion without horse). **Remaining gap:** No backend integration test proves registration creates a starter horse against the real DB. Assigned to **21R-2 Task 13** (seventh-pass correction 2026-04-13).
- [ ] [Review][Patch][High] Beta-readonly routes visible in NavPanel beta navigation. **Assigned to 21R-2 Task 11 (seventh-pass correction 2026-04-13):** NavPanel updated to filter by `isBetaLive`; beta nav shows only Home and My Stable.
- [ ] [Review][Patch][Medium] Mock-backed surfaces in production source (`MOCK_RECENT_ACTIVITY`, `MOCK_STABLE`, `MOCK_HALL_OF_FAME`, `MOCK_VET_HISTORY`). **Partially addressed:** Behind beta-hidden guards. **Assigned to 21R-2 Task 15 (seventh-pass correction 2026-04-13):** `bd` tracking issues created for each.
- [ ] [Review][Patch][Medium] No production-parity E2E test for `register → onboard → horse persisted → /stable`. **Addressed by 21R-2 Task 12 (seventh-pass correction 2026-04-13):** `tests/e2e/beta-critical-path.spec.ts` Path 1 — real credentials, no bypass, real backend, asserts horse in GET /api/horses and /stable card.
