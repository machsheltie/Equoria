...
#### Design-document loading matrix

Do **not** preload the entire design-system directory. After reading `PRODUCT.md` and `DESIGN.md` for player-facing work, load only the documents whose trigger matches the task:

- Read `docs/design-system/DECISIONS.md` when the task changes route composition, shell/navigation, headers, tabs, surfaces, dialogs, feedback architecture, shared form/state behavior, or proposes/changes a reusable UI primitive. Do not load it for copy-only, data-wiring-only, or backend work.
- Read `docs/design-system/TOKENS.md` when editing CSS, Tailwind classes, colors, typography, spacing, radii, borders, shadows, blur, z-index, or component visual variants. Do not load it for behavior-only changes that preserve existing styling.
- Read `docs/design-system/MOTION.md` when adding or changing animation, transition, loading motion, reveals, overlays, celebrations, reduced-motion behavior, or event choreography. Do not load it when the task has no motion consequence.
- Read `docs/design-system/EXCEPTIONS.md` only when the audit flags a violation, the touched file is named by an exception, the task changes the audit/baseline, or an exception is being proposed, renewed, or removed. An exception suppresses an audit match; it never grants product or design approval.
- Read `docs/design-system/inventory/README.md` only for design-system cleanup spanning multiple families, inventory maintenance, or when deciding which family inventory applies. Do not read it for routine work on one known route.
- Read `docs/design-system/inventory/foundation.md` only when changing `frontend/src/components/layout/**`, `frontend/src/components/ui/**`, global navigation/shell/background behavior, or `scripts/design-audit/**`. Loading it does not make existing shared primitives the desired design.
- Read `docs/design-system/inventory/world-services.md` only for World Hub, Veterinarian, Farrier, Feed/Tack Shops, Crafting, Grooms, Riders, or Trainers.
- Read `docs/design-system/inventory/stable-entity.md` only for Stable, horse/foal detail, equipment, lineage, genetics, traits, care history, or horse identity presentation.
- Read `docs/design-system/inventory/workflow-pages.md` only for Breeding, Training, Competition Browser/Results, Conformation Shows, or Leaderboards.
- Read `docs/diagrams/README.md` only when creating, updating, reviewing, or implementing from a wireframe, flow diagram, information-architecture map, or responsive-layout diagram. Diagrams are scoped artifacts, never product/design authority; a historical or generated diagram cannot approve a universal shell or page template. There is currently no approved canonical player-facing wireframe.
- Read `docs/features/README.md` only when creating, retiring, or auditing feature specifications, or when deciding whether a feature spec belongs in agent context. Do not load it for ordinary feature implementation when the relevant spec is already known.
- Read `docs/features/stud-service-economy.md` only when implementing or reviewing public/private stud listings, cross-owner breeding, `BreedingRequest`, stud-fee transfers, stud eligibility, or the `Equoria-e7tgc` work. Do not load it for ordinary own-horse breeding, unrelated economy/marketplace work, or general frontend styling. Its transaction, authorization, state-machine, and migration decisions are durable; its paths, issue status, code sketches, and frontend composition must be verified against current source and current product/design authority.

For a single-family task, read at most that family inventory; add `foundation.md` only if shared foundation code is also in scope. For a genuinely cross-family task, read only the inventories for the families being changed—never the directory wholesale. No inventory is design authority: it records existing implementation and active debt, and loses any conflict with the owner, `PRODUCT.md`, `DESIGN.md`, or `DECISIONS.md`.
...
**Rejected frontend dependencies** (`PRODUCT.md` § Frontend Dependency Policy — installed ≠ approved):

- **shadcn/ui and Radix UI** — rejected as Equoria's component strategy and visual default. Do not add, copy, or reinstall. `frontend/src` currently has **zero** Radix imports; keep it that way.
- **`sonner`** — not an approved feedback architecture. Do not import it or add a `toast()` call. The replacement is **Surface-Owned + Stable Log** (user decision 2026-08-19): failure → `InlineError` at the control, success → the surface's own state change plus a StableLog entry, ceremony → `CinematicMoment`. Contract: `.claude/rules/FRONTEND_ASYNC_STATE_DOCTRINE.md` § 2a. Migration: Equoria-7i4cw.
- **Recharts / Chart.js / `react-chartjs-2`** — not approved for player-facing visualization. Prefer purpose-built HTML, CSS Grid, accessible tables, timelines, ledgers, and inline SVG. Migration: Equoria-q2ols.
- **Lucide** — utility icon vocabulary only; it may clarify a control but may not carry a page's identity.

Their existing consumers are **legacy migration inventory, not endorsed architecture**. "It's already imported elsewhere" is explicitly not a reason to keep or extend one.

**The Visual-Change Gate.** Before implementing a player-facing route, answer `PRODUCT.md` § Visual-Change Gate's eight questions — chiefly: what is this route's one-sentence experiential concept, what is its emotional subject before the data, how does its silhouette differ from the generic header/tabs/cards template, and how does it use the actual scene artwork rather than sit on top of it. **If those answers are missing, stop before code.** A technically correct screen that feels like a dashboard, admin console, CRM record, or tax SaaS is a product failure, not a style quibble — and `PageHeader + Tabs + Surface/Card grid` is the named monoculture (currently 26 of 39 pages; Equoria-xgxjo).

**Legacy structure is evidence, not approval.** The 280px sidebar / 64px icon rail / 56px bottom bar, the `narrow`/`content`/`wide`/`full` containers, and the three-header-family model are **migration state**. `DESIGN.md` retired them as the settled pattern on 2026-08-19; the replacement shell needs owner-reviewed direction. Improve accessibility and correctness inside the legacy shell without treating its geometry as settled or importing a premade app-shell pattern. Repetition in the codebase is the defect the audit found — never infer "approved pattern" from it.
