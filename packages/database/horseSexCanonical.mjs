 ...
 * 'Filly', 'Colt', 'Gelding'); the marketplace store endpoint historically
...
const LOWER_TO_CANONICAL = new Map(
  CANONICAL_HORSE_SEX_VALUES.map(v => [v.toLowerCase(), v]),
);
 ...
 * 'spayed mare', 'SPAYED MARE') and returns the canonical Title Case form
 * ('Mare', 'Spayed Mare'). Trims surrounding whitespace and collapses runs
...
export function canonicalizeHorseSex(input) {
  if (typeof input !== 'string') {
    throw new TypeError(
      `canonicalizeHorseSex: expected string, got ${typeof input}`,
    );
  }
