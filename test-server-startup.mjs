...
async function testServerStartup() {
  console.log(`${colors.bold}${colors.blue}🚀 Testing Server Startup with New Routes${colors.reset}\n`);
      ...
      try {
        ...
        if (responseData.endpoints) {
          ...
          const foundEndpoints = expectedEndpoints.filter(endpoint => 
            endpoints[endpoint] && endpoints[endpoint].includes(`/api/${endpoint}`)
          );
          
          console.log(`${colors.green}✅ Found ${foundEndpoints.length}/${expectedEndpoints.length} expected new endpoints${colors.reset}`);
        ...
        console.log(`${colors.yellow}⚠️  API response not JSON, but server responded${colors.reset}`);
      }
    }
