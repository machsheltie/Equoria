...
The eligibility pre-check is advisory (READ COMMITTED race窓 exists) — that is fine by
...
| C6  | Two public breeds racing for the same mare (two different users' stallions? — no: same mare, one owner, two stallions) | exactly one pregnancy claim wins; loser 409, un-charged                                                                                           |
