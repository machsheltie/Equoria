...
## CI/Test-Infrastructure Recovery Workstream (Equoria-fefh2) — added 2026-06-10

**Context:** This workstream sits inside the tech-debt/test-quality campaign (`Equoria-fefh2`) and exists because the campaign's own verification infrastructure stopped being trustworthy. The 2026-06-10 sprint change proposal (`docs/sprint-change-proposal-2026-06-10-ci-test-infrastructure-recovery.md`, tracking issue `Equoria-4umc5`) separated the reported "full suite is red" caveat into six independent failure classes — broken fresh-database migration replay, unmeasured parallel-Jest contention, a deterministic CSRF defect (`Equoria-lax36`, already fixed), backend lint errors, stale executable evidence (`Equoria-veql`), and a double-configured CodeQL setup — and mandated a dependency-ordered recovery program ahead of any further campaign-closure claims.

**Rule:** targeted-suite green runs are NOT closure evidence for any story in this group. Closure requires the authoritative gate evidence defined in the proposal's Definition of Done (Section 10).

| Story            | Title                                                                                                                                                                                                                        | Status (2026-06-10)                                                                                                                                                                                                   |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Equoria-fefh2.14 | Repair fresh-database migration chain (v58ta horses FKs + email-verification-tokens FK amended to DROP CONSTRAINT IF EXISTS + ADD; new replay sentinel `backend/__tests__/scripts/freshDbMigrationReplay.sentinel.test.mjs`) | done — pending merge to master                                                                                                                                                                                        |
| Equoria-fefh2.15 | Diagnose full parallel Jest `fetchCsrf` timeout wave (worker-count matrix, measured root cause)                                                                                                                              | in progress — root cause measured as pre-suite-hook Prisma teardown ordering; peak connections 101→8 and too-many-clients 5,918→0; named profiles + sync/budget sentinels landed locally; full-run/CI burn-in pending |
| Equoria-fefh2.16 | Fix `legacyUserDelete` per-user CSRF binding + cleanup (the `Equoria-lax36` defect; lax36 remains open pending user closure)                                                                                                 | done — identity-bound per-user CSRF, fail-loud FK-ordered cleanup, 3/3 green                                                                                                                                          |
| Equoria-fefh2.17 | Clear backend lint errors                                                                                                                                                                                                    | done — 0 errors; 79 warnings tracked separately under `Equoria-fefh2.23`                                                                                                                                              |
| Equoria-fefh2.18 | Re-establish `Equoria-veql` evidence                                                                                                                                                                                         | done — re-pointed at the canonical scan library `scripts/lib/beta-readiness-scans.sh` (ADR-010), sentinel-positive proof retained                                                                                     |
| Equoria-fefh2.19 | Resolve CodeQL default/advanced conflict + v4 upgrade                                                                                                                                                                        | done — advanced workflow (`.github/workflows/codeql.yml`) is the sole owner; GitHub default setup disabled 2026-06-10; `codeql-action` v3→v4                                                                          |
| Equoria-fefh2.20 | Restore complete master gate and retire the `--no-verify` push exception                                                                                                                                                     | blocked — **strictly last**; depends on .14–.19 all landed and green                                                                                                                                                  |

**Dependency rule:** `.14`–`.19` may proceed in parallel where they do not touch the same files; `.20` runs strictly last and is the only story that can retire the CLAUDE.md `--no-verify` active exception (user removes it, per Constitution §6/§7). Follow-up issues `Equoria-fefh2.21`–`.26` track non-blocking remainders surfaced during recovery, including the lint-warning burn-down (`.23`) and the ~27-suite anonymous-CSRF-then-authenticated-mutation triage (`.25`; per-user CSRF binding under `Equoria-plw0h`).

**Known production state (2026-06-10):** the production database (Supabase via Railway) has migration `20260530120000_v58ta_horse_restrict_fks` recorded as **failed** since 2026-05-30, blocking deploys. The repair (`prisma migrate resolve --rolled-back` + `migrate deploy`) is proven on a simulation and awaits explicit user authorization before being run against production (Equoria-fefh2.14 scope).
