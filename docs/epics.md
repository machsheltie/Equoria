...
| Equoria-fefh2.15 | Diagnose full parallel Jest `fetchCsrf` timeout wave (worker-count matrix, measured root cause)                                                                                                                              | in progress — root cause NOT yet measured; named profiles `test:backend:full/ci/targeted/diagnostic` are planned, not landed                 |
