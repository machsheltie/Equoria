 ...
 */
async function breedHorses(sireId: string, damId: string, name: string): Promise<Horse> {
  // Implementation
}
```
...
const expensiveCalculation = useMemo(() => calculateGeneticProbabilities(horse), [horse]);
```
