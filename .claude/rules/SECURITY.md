...
#### **CSRF Protection — Double-Submit + Cookie-vs-Header Binding Contract (Equoria-plw0h / Equoria-lax36)**

CSRF is enforced by the double-submit cookie pattern in `backend/middleware/csrf.mjs` (the `csrf-csrf` library): the client sends the token in the `X-CSRF-Token` header and the browser sends the matching cookie automatically; the library HMACs and compares them on every POST/PUT/PATCH/DELETE. The second line of defense (Equoria-plw0h) is per-user session binding — `resolveSessionIdentifier(req)` salts the HMAC with `req.user.id` so a token minted under user A cannot validate under user B's session, even if a sub-vulnerability (subdomain XSS, sibling-app cookie injection on the same eTLD+1) plants a cookie+header pair in the victim's browser.

**The binding is intentionally applied ONLY to COOKIE-authenticated sessions, NOT to Bearer-header-authenticated requests.** `authenticateToken` (`backend/middleware/auth.mjs`) records the JWT's source on `req.authTokenSource` (`'cookie'` when the access token came from the cookie, `'header'` when it came from an `Authorization: Bearer` header). `resolveSessionIdentifier` skips the `req.user.id` binding when `req.authTokenSource === 'header'`, falling back to the refresh-cookie / salt identifier the token was issued under. This is deliberate, not an oversight: a Bearer-header request is structurally NOT CSRF-able — a browser never auto-attaches an `Authorization` header to a forged cross-site request, so the attacker cannot make the victim send one. Binding header auth to `req.user.id` would gain nothing against the CSRF threat model while breaking every legitimate header-auth API client (mobile / server-to-server / the documented backward-compat header path): such clients fetch a CSRF token anonymously (bound to the salt at issuance) then mutate with a Bearer header (resolving to `req.user.id` at validation), and the identifier mismatch would 403 every legitimate mutation — exactly the Equoria-plw0h regression that Equoria-lax36 fixed. The cookie-session cross-user-replay defense is fully preserved, because a cookie-authenticated request is the only CSRF-able case and still resolves to `req.user.id`.

**Contract for future edits:** do NOT re-bind `req.authTokenSource === 'header'` requests to `req.user.id` in `resolveSessionIdentifier` — doing so re-introduces the plw0h 403 regression for all header-auth clients. This contract is guarded by the sentinel test `backend/modules/auth/__tests__/csrfBearerHeaderBinding.sentinel.test.mjs` (real DB, real Express, no mocks): it asserts a Bearer-header mutation with an anonymously-fetched CSRF token succeeds (200, not 403), that a Bearer-header mutation with NO CSRF token still fails closed (403), and that a cookie session replaying another user's CSRF token is still rejected (403, preserved plw0h protection).
