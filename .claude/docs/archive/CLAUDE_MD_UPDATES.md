...
{
  "hooks": {
    "pre-test": ["echo 'Running tests with Day 3 Phase 1 standards...'", "jest --clearCache"],
    "post-test": [
      "echo 'Verifying no test warnings...'",
      "grep -q 'Warning:' test-output.log && exit 1 || exit 0"
    ],
    "pre-commit-frontend": ["cd frontend && npm run test", "cd frontend && npm run test:coverage"],
