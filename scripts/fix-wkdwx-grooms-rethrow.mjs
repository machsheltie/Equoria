#!/usr/bin/env node
/**
 * Equoria-wkdwx grooms-services-slice one-shot codemod.
 *
 * Removes function-body `try { ... } catch (error) { logger.error(...); throw error; }`
 * wrappers in the 8 backend/modules/grooms/services/*.mjs files carried in the
 ...
 * `Error ... ${error.message}` already implied by the stack trace.
 *
 * This mirrors the approved precedent codemod
 * scripts/fix-wkdwx-breeding-rethrow.mjs (modelled on commit 746fd6384)
 * byte-for-byte in strategy; only the FILES list differs.
 *
 * Catches that do anything beyond log-then-throw are left untouched by the
 * isLogThenThrow guard below — specifically the fail-soft catches in this
 * slice (groomRetirementService.processRetirement → legacy auto-create catch
 * that does NOT rethrow; weeklyCareerProgression inner per-groom catch that
 * pushes to results.errors and does NOT rethrow) have no matching
 * `throw error;`, and the weeklyCareerProgression outer catch transforms the
 * error (`throw new Error(...)`) rather than rethrowing the same value — all
 * three are correctly skipped, so behaviour is unchanged for them.
 *
...
const FILES = [
  'backend/modules/grooms/services/enhancedGroomInteractions.mjs',
  'backend/modules/grooms/services/groomAssignmentService.mjs',
  'backend/modules/grooms/services/groomBonusTraitService.mjs',
  'backend/modules/grooms/services/groomLegacyService.mjs',
  'backend/modules/grooms/services/groomPerformanceService.mjs',
  'backend/modules/grooms/services/groomPersonalityTraits.mjs',
  'backend/modules/grooms/services/groomRetirementService.mjs',
  'backend/modules/grooms/services/groomTalentService.mjs',
];
...
console.log('=== Equoria-wkdwx grooms-services slice codemod ===');
let total = 0;
