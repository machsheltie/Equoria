#!/usr/bin/env node
/**
 * Equoria-678jm one-shot codemod.
 *
 * Removes function-body `try { ... } catch (error) { logger.error(...); throw error; }`
 * wrappers in the 5 backend/services/genetics/*.mjs files identified by the
 * rethrow-after-log doctrine check. Per OPTIMAL_FIX §3 the global error handler
...
console.log('=== Equoria-678jm codemod ===');
let total = 0;
