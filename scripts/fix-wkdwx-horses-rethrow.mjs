#!/usr/bin/env node
/**
 * Equoria-wkdwx horses-services-slice one-shot codemod.
 *
 * Removes function-body `try { ... } catch (error) { logger.error(...); throw error; }`
 * wrappers in the backend/modules/horses/services/*.mjs files carried in the
 ...
 * scripts/fix-wkdwx-grooms-rethrow.mjs and scripts/fix-wkdwx-breeding-rethrow.mjs
 ...
 * untouched by the isLogThenThrow guard below — specifically:
 *   - horseModelService.createHorse / getHorseById / updateDisciplineScore /
 *     getDisciplineScores transform the error (`throw new Error(...)`) rather
 *     than rethrowing the same value — all four are correctly skipped (the
 *     guard excludes `throw new`), so behaviour is unchanged for them. The
 *     transformed message is a genuine context wrapper, not a no-op rethrow.
 *   - developmentalWindowSystem.mjs and personalityEvolutionSystem.mjs are NOT
 *     in this slice's FILES list (deferred — see wkdwx notes / remainder).
 *
...
const FILES = [
  'backend/modules/horses/services/horseModelService.mjs',
  'backend/modules/horses/services/horseTemperamentAnalysis.mjs',
  'backend/modules/horses/services/legacyScoreCalculator.mjs',
];
...
console.log('=== Equoria-wkdwx horses-services slice codemod ===');
let total = 0;
