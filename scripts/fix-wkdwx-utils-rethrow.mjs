#!/usr/bin/env node
/**
 * Equoria-wkdwx utils-slice one-shot codemod.
 *
 * Removes function-body `try { ... } catch (error) { logger.error(...); throw error; }`
 * wrappers in the backend/utils/*.mjs files carried in the rethrow-after-log
 * doctrine baseline (Equoria-ej9k1). Per OPTIMAL_FIX §3 the global error handler
 * already logs once with full context, so the try/catch wrapper adds nothing —
 * the catch bodies here only re-log a generic `Error ... ${error.message}`
 * already implied by the stack trace.
 *
 * This mirrors the approved precedent codemods
 * scripts/fix-wkdwx-horses-rethrow.mjs and scripts/fix-wkdwx-labs-rethrow.mjs
 * (modelled on commit 746fd6384) byte-for-byte in strategy; only the FILES list
 * differs.
 *
 * The isLogThenThrow guard leaves untouched any catch that does more than
 * log-then-throw-the-SAME-value: error transformation (`throw new`), conditional
 * branches (`if (`), or async cleanup (`await `). Catches at deeper-than-function
 * indent are also left for manual review (the matcher only acts on the
 * 2-space-indent function-body `try {`).
 *
 * NOT in this slice's FILES list (deferred — see wkdwx remainder):
 *   backend/services/cronJobs.mjs (intentional cron logging, separate review).
 *
...
const FILES = [
  'backend/utils/applyEpigeneticTraitsAtBirth.mjs',
  'backend/utils/carePatternAnalysis.mjs',
  'backend/utils/dailyCareAutomation.mjs',
  'backend/utils/emailService.mjs',
  'backend/utils/emailVerificationService.mjs',
  'backend/utils/enhancedMilestoneEvaluationSystem.mjs',
  'backend/utils/flagEvaluationEngine.mjs',
  'backend/utils/foalTaskLogManager.mjs',
  'backend/utils/groomBondingSystem.mjs',
  'backend/utils/groomRareTraitPerks.mjs',
  'backend/utils/groomSystem.mjs',
  'backend/utils/horseAgingSystem.mjs',
  'backend/utils/horseModelTraitHelpers.mjs',
  'backend/utils/horseUpdates.mjs',
  'backend/utils/milestoneTraitEvaluator.mjs',
  'backend/utils/tokenRotationService.mjs',
  'backend/utils/traitAssignmentLogic.mjs',
  'backend/utils/traitDiscovery.mjs',
  'backend/utils/traitEvaluation.mjs',
  'backend/utils/ultraRareTriggerEngine.mjs',
  'backend/utils/userUpdates.mjs',
];
...
console.log('=== Equoria-wkdwx utils slice codemod ===');
let total = 0;
