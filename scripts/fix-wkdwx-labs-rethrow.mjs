#!/usr/bin/env node
/**
 * Equoria-wkdwx labs-services-slice one-shot codemod.
 *
 * Removes function-body `try { ... } catch (error) { logger.error(...); throw error; }`
 * wrappers in the 2 backend/modules/labs/services/*.mjs files carried in the
 * rethrow-after-log doctrine baseline (Equoria-ej9k1):
 *   - environmentalFactorEngineService.mjs (baseline 8)
 *   - environmentalTriggerSystem.mjs        (baseline 8)
 * Per OPTIMAL_FIX §3 the global error handler already logs once with full
 * context, so the try/catch wrapper adds nothing — the catch bodies here only
 * re-log a generic `Error ... ${error.message}` / `Error ... :, error` already
 * implied by the stack trace, then rethrow the SAME value.
 *
 * This mirrors the approved precedent codemods
 * scripts/fix-wkdwx-grooms-rethrow.mjs / fix-wkdwx-breeding-rethrow.mjs /
 * fix-wkdwx-horses-rethrow.mjs (modelled on commit 746fd6384) byte-for-byte in
 * strategy; only the FILES list + comments differ.
 *
 * Catches that do anything beyond log-then-throw-the-SAME-value are left
 * untouched by the isLogThenThrow guard below — specifically the fail-soft
 * catches in the sibling labs file enhancedReportingService.mjs (one logs+
 * continues with no throw; one logs+`return detailed;`) are NOT in this slice's
 * FILES list and never matched anyway (no `throw error;`). The other two labs
 * service files (enhancedReportingQueries.mjs — no throws; environmentalHorseService.mjs
 * — no catch blocks) carry no rethrow-after-log and are not in the baseline.
 *
 * The two FILES below pass `error` as a second arg in some catches
 * (`logger.error(\`...\`, error)`); that is still a pure log-then-throw — the
 * second arg is richer logging, not behaviour — so the guard correctly matches
 * (it forbids `throw new`, `if (`, and `await` in the catch body, none of which
 * appear).
 *
...
const FILES = [
  'backend/modules/labs/services/environmentalFactorEngineService.mjs',
  'backend/modules/labs/services/environmentalTriggerSystem.mjs',
];
...
console.log('=== Equoria-wkdwx labs-services slice codemod ===');
let total = 0;
