...
import {
  ...
  getRevealedPerks,
  applyRareTraitBoosterEffects as _applyRareTraitBoosterEffects,
...
router.post(
  ...
  [
    ...
    body('evaluationContext')
      .optional()
      .isObject()
      .withMessage('Evaluation context must be an object'),
  ],
  validateRequest,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!groom) {
        logger.error(
          '[ultraRareTraitRoutes.assign-perks] Groom not found after ownership validation',
        );
        return res.status(404).json({
          success: false,
...
router.post(
  ...
  [
    ...
    body('effectType')
      .isIn(['stress', 'competition', 'bonding', 'training'])
  ...
  async (req, res) => {
    try {
      ...
      switch (effectType) {
        case 'stress':
          effectResult = applyUltraRareStressEffects(
            horse,
            parseFloat(baseValue),
            context.stressSource,
          );
          break;
        case 'competition':
