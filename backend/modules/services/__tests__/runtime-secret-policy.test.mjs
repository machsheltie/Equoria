...
const importFresh = path => import(`${path}?cacheBust=${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
describe('deployable runtime secret policy', () => {
  ...
  it('script-level validation fails deployable modes that use committed test-only refresh secrets', () => {
    ...
    expect(results.errors).toEqual(
      expect.arrayContaining([expect.stringContaining('JWT_REFRESH_SECRET uses a committed test-only secret')]),
    );
  });
});
