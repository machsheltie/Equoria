      ...
      async () => {
        ...
        return topScores.map((entry, index) => {
          ...
          return {
            ...
            owner: horse?.user?.username || 'Unknown',
...
export const getTopHorsesByEarnings = async (req, res) => {
  ...
  try {
    ...
    const rankedHorses = horses.map((horse, index) => ({
      ...
      ownerName: horse.user?.username || 'Unknown',
...
export const getRecentWinners = async (req, res) => {
  try {
    ...
    const winners = results.map(entry => ({
      ...
      owner: entry.horse?.user?.username || 'Unknown Owner',
    ...
    const rankings = [
      {
        ...
        rankChange: snapshotMap['level'] !== undefined ? snapshotMap['level'] - levelRank : 0,
      ...
      },
      {
        ...
        totalEntries: totalUsers,
        rankChange: snapshotMap['xp'] !== undefined ? snapshotMap['xp'] - xpRank : 0,
      ...
      },
      {
        ...
        rankChange:
          snapshotMap['horse-earnings'] !== undefined
            ? snapshotMap['horse-earnings'] - earningsRank
            : 0,
      ...
      },
      {
        ...
        rankChange:
          snapshotMap['horse-performance'] !== undefined
