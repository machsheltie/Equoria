...
export async function getGroomAnalytics(req, res) {
  try {
    ...
    logger.info(
      `[groomPerformanceController] Getting analytics for groom ${groomId}, last ${days} days`,
    );
    ...
    const analytics = {
      ...
      averageBondGain:
        records.length > 0 ? records.reduce((sum, r) => sum + r.bondGain, 0) / records.length : 0,
      successRate:
        records.length > 0 ? (records.filter(r => r.taskSuccess).length / records.length) * 100 : 0,
      averageWellbeingImpact:
        records.length > 0
          ? records.reduce((sum, r) => sum + r.wellbeingImpact, 0) / records.length
          : 0,
      averageDuration:
        records.length > 0 ? records.reduce((sum, r) => sum + r.duration, 0) / records.length : 0,
      interactionTypes: {},
      dailyActivity: {},
      horsesWorkedWith: [],
    };
    ...
    const uniqueHorses = [
      ...new Map(records.filter(r => r.horse).map(r => [r.horse.id, r.horse])).values(),
    ];
