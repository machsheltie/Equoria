...
export async function createGroomAssignment(req, res) {
  try {
    ...
    logger.info(
      `[groomAssignmentController] Creating assignment: groom ${groomId} -> horse ${horseId}`,
    );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        summary: {
          ...
          averageCostPerGroom:
            costs.groomCount > 0 ? Math.round(costs.totalWeeklyCost / costs.groomCount) : 0,
        },
      },
    });
  } catch (error) {
...
export async function validateAssignment(req, res) {
  try {
    ...
    logger.info(
      `[groomAssignmentController] Validating assignment: groom ${groomId} -> horse ${horseId}`,
    );
      ...
      data: {
        ...
        summary: {
          ...
          averageAssignmentsPerGroom:
            Math.round(assignmentsResult.stats.averageAssignmentsPerGroom * 10) / 10,
        },
      },
    });
  } catch (error) {
    logger.error(
      `[groomAssignmentController] Error getting assignment dashboard: ${error.message}`,
    );
    res.status(500).json({
      success: false,
      message: 'Internal server error',
