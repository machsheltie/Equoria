/**
 * competitionController — bare-default stat-fallback sentinel (Equoria-182zv).
 *
 * Asserts that
 * backend/modules/competition/controllers/competitionController.mjs never
 * reintroduces the bare-default `horse.<statField> || <number>` pattern when
 * reading Horse stat columns. Horse stats are nullable Int columns (0..100);
 * a stat of 0 is a legitimate game state (undeveloped or injury). The `||`
 * fallback silently masks 0 by collapsing it to the fallback — for non-zero
 * fallbacks this corrupts the score; for `|| 0` it is numerically harmless
 * but defeats the canonical one-form rule and risks drift if the fallback
 * is ever raised.
 *
 ...
 * pattern is reintroduced in the controller file.
 *
 * Cross-reference:
 ...
 *   environmentalRoutes (Equoria-hnci3),
...
describe('Equoria-182zv — bare-default stat-fallback sentinel for competitionController', () => {
