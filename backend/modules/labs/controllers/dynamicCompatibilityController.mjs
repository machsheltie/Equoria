...
export async function calculateCompatibility(req, res) {
  try {
    ...
    logger.info(
      `[dynamicCompatibilityController.calculateCompatibility] Calculating compatibility for groom ${groomId} and horse ${horseId}`,
    );
    ...
    logger.info(
      `[dynamicCompatibilityController.calculateCompatibility] Compatibility calculated: ${compatibility.overallScore.toFixed(3)} (${compatibility.recommendationLevel})`,
    );
...
export async function getCompatibilityFactors(req, res) {
  try {
    ...
    logger.info(
      `[dynamicCompatibilityController.getCompatibilityFactors] Analyzing factors for groom ${groomId} and horse ${horseId}`,
    );

    const factors = await analyzeCompatibilityFactors(parseInt(groomId), parseInt(horseId));

    logger.info(
      `[dynamicCompatibilityController.getCompatibilityFactors] Factors analyzed: ${factors.overallAssessment.rating} compatibility`,
    );
...
export async function predictOutcome(req, res) {
  try {
    ...
    logger.info(
      `[dynamicCompatibilityController.predictOutcome] Predicting outcome for groom ${groomId} and horse ${horseId}`,
    );
    ...
    logger.info(
      `[dynamicCompatibilityController.predictOutcome] Outcome predicted: ${prediction.predictedQuality} quality, ${prediction.successProbability.toFixed(2)} success probability`,
    );
...
export async function getRecommendations(req, res) {
  try {
    ...
    logger.info(
      `[dynamicCompatibilityController.getRecommendations] Getting recommendations for horse ${horseId}`,
    );
    ...
    logger.info(
      `[dynamicCompatibilityController.getRecommendations] Found ${recommendations.rankedGrooms.length} groom recommendations`,
    );
...
export async function getCompatibilityTrends(req, res) {
  try {
    ...
    logger.info(
      `[dynamicCompatibilityController.getCompatibilityTrends] Analyzing trends for groom ${groomId} and horse ${horseId}`,
    );

    const trends = await analyzeCompatibilityTrends(parseInt(groomId), parseInt(horseId));

    logger.info(
      `[dynamicCompatibilityController.getCompatibilityTrends] Trends analyzed: ${trends.overallTrend} trend with ${trends.dataPoints} data points`,
    );
...
export async function getCompatibilityConfig(req, res) {
  try {
    logger.info(
      '[dynamicCompatibilityController.getCompatibilityConfig] Getting compatibility system configuration',
    );
