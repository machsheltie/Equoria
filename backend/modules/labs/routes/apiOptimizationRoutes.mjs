 ...
 */
router.get('/metrics', authenticateToken, getOptimizationMetrics);
 ...
 */
router.get('/compression-stats', authenticateToken, async (req, res) => {
  try {
...
router.post(
  ...
  body('dataSize')
    ...
    .withMessage('Data size must be between 1 and 10000'),
  body('pageSize')
    .optional()
    .isInt({ min: 1, max: 100 })
  ...
  body('paginationType')
    .optional()
    .isIn(['offset', 'cursor'])
    .withMessage('Invalid pagination type'),
  validateRequest,
  async (req, res) => {
    try {
...
function generateCompressionRecommendation(ratio) {
  if (ratio > 0.7) {
    return 'Excellent compression ratio';
  }
  if (ratio > 0.5) {
    return 'Good compression ratio';
  }
  if (ratio > 0.3) {
    return 'Moderate compression ratio';
  }
...
function estimateImpact(recommendations) {
  return {
    performance:
      recommendations.length > 3 ? 'high' : recommendations.length > 1 ? 'medium' : 'low',
    bandwidth: recommendations.some(r => r.includes('compression')) ? 'high' : 'low',
    userExperience: recommendations.some(r => r.includes('cache') || r.includes('response'))
      ? 'high'
      : 'medium',
  };
}
