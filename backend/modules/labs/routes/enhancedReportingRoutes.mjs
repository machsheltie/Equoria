...
import {
  ...
  analyzeTraitTrends as _analyzeTraitTrends,
  identifyTraitPatterns as _identifyTraitPatterns,
  generateTrendPredictions as _generateTrendPredictions,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (discoveryMethod) {
        whereConditions.sourceType = discoveryMethod;
      } // Map discoveryMethod to sourceType
      if (dateFrom || dateTo) {
 ...
 */
router.get(
  '/users/:id/stable-epigenetic-report',
  authenticateToken,
  ...
  async (req, res) => {
    try {
      ...
      const recommendations = generateStableRecommendations(
        stableOverview,
        traitDistribution,
        developmentalStages,
      );
    ...
    try {
      ...
      if (horses === null) {
        ...
        return res.status(404).json({
          success: false,
          message: 'One or more horses not found',
        });
      }
 ...
 */
router.get(
  '/horses/:id/epigenetic-report-export',
  authenticateToken,
  param('id').isInt({ min: 1 }).withMessage('Horse ID must be a positive integer'),
  query('format')
    .optional()
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `Epigenetic report export generated for horse ${horseId} (${format}) by user ${req.user.id}`,
      );
