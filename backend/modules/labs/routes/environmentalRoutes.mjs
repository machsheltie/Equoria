...
router.get(
  ...
  [
    ...
    query('region')
      .optional()
      .isIn(['tropical', 'temperate', 'arctic', 'desert', 'coastal'])
      .withMessage('Invalid region'),
    query('elevation')
      .optional()
      .isInt({ min: 0 })
      .withMessage('Elevation must be a positive integer'),
  ],
  validateRequest,
  async (req, res) => {
    try {
      ...
      logger.info(
        `[environmentalRoutes.current] Getting current conditions for ${location.latitude}, ${location.longitude}`,
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `[environmentalRoutes.forecast] Generating ${days}-day forecast for ${location.latitude}, ${location.longitude}`,
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `[environmentalRoutes.calculate-impact] Calculating impact for ${horses.length} horses`,
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `[environmentalRoutes.comfort-zone] Calculating comfort zone for horse ${horse.id}`,
      );
