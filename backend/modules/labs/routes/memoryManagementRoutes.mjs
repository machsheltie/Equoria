...
import {
  ...
  trackResource as _trackResource,
  untrackResource as _untrackResource,
      ...
      const analytics = {
        averageHeapUsed:
          filteredMetrics.length > 0
            ? filteredMetrics.reduce((sum, m) => sum + m.heapUsed, 0) / filteredMetrics.length
            : 0,
        peakHeapUsed:
          filteredMetrics.length > 0 ? Math.max(...filteredMetrics.map(m => m.heapUsed)) : 0,
        averageHeapUtilization:
          filteredMetrics.length > 0
            ? filteredMetrics.reduce((sum, m) => sum + m.heapUtilization, 0) /
...
function generateHealthRecommendations(report, memUsage) {
  ...
  if (memUtilization > 0.8) {
    recommendations.push(
      'Memory utilization is high - consider optimizing memory usage or increasing heap size',
    );
  }
