/**
 * POST /api/tack-shop/purchase — recordTransactionTx migration sentinel
 * (Equoria-0caxg).
 *
 * Per Equoria-pqp69 migration plan: the tack-shop purchase path was migrated
 * from the legacy `recordTransaction(opts, tx)` signature to
 ...
 *
 *   1. Purchasing a real tack item via the controller (no HTTP — same
 *      pattern as tackPurchaseConcurrentRace, dodges the pre-existing CSRF
 *      flake Equoria-pyz4z).
 ...
 *      money column AFTER the debit (i.e. starting money minus item cost).
 *
 ...
 * correct category, amount, type, and metadata.{horseId,itemId,category}
 * so a refactor that drops one of those fields fails loudly.
 *
 * Real DB, no mocks, scoped fixtures (TestFixture-0caxg prefix).
 */
...
const FIXTURE_PREFIX = 'TestFixture-0caxg-tack';
...
beforeAll(async () => {
  ...
  // Pick the first non-legacy functional item with a positive cost.
  item = TACK_INVENTORY.filter(i => !i.isLegacyAlias && i.cost > 0)[0];
...
describe('tackShop purchaseTackItem recordTransactionTx migration sentinel (Equoria-0caxg)', () => {
  it('SENTINEL: ledger row balanceAfter is sourced from inside the tx (post-debit)', async () => {
    ...
    const expectedMoneyAfter = STARTING_MONEY - item.cost;
    ...
    expect(Number(ledgerRow.amount)).toBe(item.cost);
    expect(ledgerRow.category).toBe('tack_purchase');
