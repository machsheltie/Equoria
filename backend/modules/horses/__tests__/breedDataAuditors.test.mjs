...
const BREEDS_DIR = join(REPO_ROOT, 'samples', 'Breeds');

function runAuditor(scriptName) {
  const scriptPath = join(BREEDS_DIR, scriptName);
  return execFileSync(process.execPath, [scriptPath], {
    encoding: 'utf8',
    env: { ...process.env, BREED_DIR: BREEDS_DIR },
...
describe('breed-data auditors (Equoria-26qjf.4 AC3 — data guard)', () => {
  ...
  test('_audit_base_color.mjs: 0 implausible, 0 map-keys-not-found', () => {
    ...
    const m = out.match(
      /BASE-COLOR AUDIT:\s*(\d+)\s*plausible,\s*(\d+)\s*implausible,\s*(\d+)\s*map-keys-not-found/,
    );
    expect(m).not.toBeNull();
