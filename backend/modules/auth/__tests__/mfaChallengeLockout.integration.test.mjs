  ...
  it('lockout is per-userId — User B is not affected by User A being locked', async () => {
    ...
    try {
      ...
      const enrollB = await request(app)
        .post('/api/v1/auth/mfa/enroll')
        .set('Cookie', [...csrf.cookieHeader, ...cookiesB])
        .set('X-CSRF-Token', csrf.csrfToken)
        .send({});
      ...
      const challengeB = (await request(app)
        .post('/api/v1/auth/login')
        .set('Cookie', csrf.cookieHeader)
        .set('X-CSRF-Token', csrf.csrfToken)
        .send({ email: userB.email, password: userB.plainPassword })).body.data.mfaChallengeToken;
