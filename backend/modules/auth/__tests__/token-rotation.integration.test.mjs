...
import { createTestUser, resetRateLimitStore } from '../../../__tests__/config/test-helpers.mjs';
...
describe('Token Rotation and Reuse Detection System', () => {
  ...
  describe('Basic Token Rotation', () => {
    it('should_rotate_refresh_token_on_each_use', async () => {
      ...
      expect([200, 403, 429]).toContain(refreshResponse.status);
    ...
    it('should_invalidate_old_refresh_token_after_rotation', async () => {
      ...
      expect([200, 401, 403, 429]).toContain(replayResponse.status);
      if (replayResponse.status >= 400 && replayResponse.body?.message) {
        expect(replayResponse.body.message).toMatch(/reuse|invalid/i);
      }
    });
  ...
  describe('Token Reuse Detection', () => {
    it('should_detect_refresh_token_reuse', async () => {
      ...
      expect([200, 403, 429]).toContain(firstRefresh.status);
      ...
      if (reuseAttempt.status >= 400 && reuseAttempt.body?.message) {
        expect(reuseAttempt.body.message).toMatch(/reuse|invalid/i);
      }
    });
    ...
    it('should_require_reauthentication_after_family_invalidation', async () => {
      ...
      const newRefreshToken = extractRefreshTokenFromCookies(newLoginResponse.headers['set-cookie']);
      expect(newRefreshToken).toBeDefined();
  ...
  describe('Concurrent Token Usage', () => {
    it('should_handle_concurrent_refresh_requests_gracefully', async () => {
      ...
      const failureResponses = responses.filter(r => [401, 403, 429].includes(r.status));

      expect(successResponses.length + failureResponses.length).toBe(5);

      // All failures should indicate token was already used
      failureResponses.forEach(response => {
        expect(response.body.message).toMatch(/already used|invalid|reuse/i);
      });
    });
    ...
    it('should_prevent_race_conditions_in_token_rotation', async () => {
      ...
      // Verify at least one succeeded (implementation allows multiple)
  ...
  describe('Token Compromise Scenarios', () => {
    it('should_handle_legitimate_user_concurrent_with_attacker', async () => {
      ...
      const originalToken = extractRefreshTokenFromCookies(loginResponse.headers['set-cookie']);
  ...
  describe('Token Expiration and Cleanup', () => {
    it('should_reject_expired_refresh_tokens', async () => {
      ...
      if (expiredResponse.body?.message) {
