 ...
 * - styleSrc still allows `'unsafe-inline'`. The former accepted-risk ADR was
 *   retired because its Radix/react-remove-scroll rationale no longer matches
 *   application source. Treat this allowance as unresolved legacy CSP debt:
 *   audit actual runtime style injection before tightening it. script-src
 *   remains 'self'-only. A security sentinel locks the current shape so it
 *   cannot silently broaden; the sentinel is evidence of behavior, not proof
 *   that the residual risk is permanently accepted.
 */
