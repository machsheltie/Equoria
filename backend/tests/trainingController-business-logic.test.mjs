...
describe('🏋️ INTEGRATION: Training Controller Business Logic - Complete Training Workflow', () => {
  ...
  describe('BUSINESS RULE: trainHorse() Function Complete Workflow', () => {
    it('EXECUTES successful training workflow for eligible horse', async () => {
      ...
      // Cross-VM-realm safe Date check: toBeInstanceOf(Date) fails with
      // --experimental-vm-modules because each module gets its own Date constructor.
      expect(Object.prototype.toString.call(trainingLog.trainedAt)).toBe('[object Date]');
      ...
      // Cross-VM-realm safe Date check (toBeInstanceOf/expect.any(Date) fails with --experimental-vm-modules)
      expect(Object.prototype.toString.call(status.lastTrainingDate)).toBe('[object Date]');
      expect(status).toEqual(
        expect.objectContaining({
          eligible: false, // Fixed: match actual API structure
          reason: expect.any(String),
          horseAge: expect.any(Number),
          lastTrainingDate: expect.anything(), // Date check done separately above
  ...
  describe('BUSINESS RULE: getTrainingStatus() Function Validation', () => {
    ...
    it('RETURNS accurate status for horse with training history', async () => {
      ...
      // Cross-VM-realm safe Date check (toBeInstanceOf/expect.any(Date) fails with --experimental-vm-modules)
      expect(Object.prototype.toString.call(result.lastTrainingDate)).toBe('[object Date]');
      expect(result).toEqual(
        expect.objectContaining({
          eligible: false, // Fixed: match actual API structure
          reason: expect.any(String),
