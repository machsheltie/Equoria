...
describe('Groom Personality Trait Bonus System - REAL SYSTEM TESTS', () => {
  ...
  describe('Compatible Grooms Utility', () => {
    it('should return all compatible grooms for a temperament', () => {
      ...
      expect(compatibleGrooms.some(g => g.personality === GROOM_PERSONALITY_TYPES.CALM)).toBe(true);
      expect(compatibleGrooms.some(g => g.personality === GROOM_PERSONALITY_TYPES.SOFT_SPOKEN)).toBe(true);
      expect(compatibleGrooms.some(g => g.personality === GROOM_PERSONALITY_TYPES.RESERVED)).toBe(true);
    });
  ...
  describe('API Endpoints - REAL HTTP TESTS', () => {
    ...
    describe('POST /api/milestones/evaluate-milestone (with personality impact)', () => {
      it('should evaluate milestone with personality effects', async () => {
        ...
        expect(response.body.data.modifiersApplied.personalityEffects.personalityMatchScore).toBe(2); // Strong match
        expect(response.body.data.modifiersApplied.personalityEffects.personalityEffectApplied).toBe(true);
        expect(response.body.data.modifiersApplied.personalityEffects.personalityCompatibility.isMatch).toBe(true);
      });
    });
  });
});
