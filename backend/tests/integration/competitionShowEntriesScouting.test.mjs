/**
 * GET /api/competition/show/:showId/entries — Scouting endpoint integration
 ...
 *
 * The whole /api/competition router is mounted under the app's authRouter
...
describe('INTEGRATION: GET /api/competition/show/:showId/entries (Equoria-lfkw1)', () => {
  let user;
  let authToken;
  let horseA;
  let horseB;
  ...
  afterAll(async () => {
    await prisma.showEntry.deleteMany({ where: { id: { in: createdEntryIds } } }).catch(() => {});
    await cleanupTestData();
  });

  it('returns the real entered field with breed/level/top-3-stats/owner + header', async () => {
    const res = await get(`/api/competition/show/${show.id}/entries`);
  ...
  it('returns an empty field (count 0) for a show with no entries', async () => {
    ...
    const res = await get(`/api/competition/show/${emptyShow.id}/entries`);
    expect(res.status).toBe(200);
  ...
  it('404s for a non-existent show', async () => {
    const res = await get('/api/competition/show/999999999/entries');
    expect(res.status).toBe(404);
    expect(res.body.success).toBe(false);
  });

  it('400s for an invalid show id', async () => {
    const res = await get('/api/competition/show/not-a-number/entries');
