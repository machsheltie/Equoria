...
import { describe, test, expect, beforeEach, afterEach as _afterEach } from '@jest/globals';
import _logger from '../../utils/logger.mjs';
import {
...
describe('🌤️ Environmental Factor Engine', () => {
  ...
  describe('⚡ Environmental Triggers', () => {
    ...
    test('should identify stress triggers based on weather conditions', () => {
      ...
      const stressTriggersExtreme = calculateEnvironmentalTriggers(testDate, testLocation, testHorse, extremeWeather);
      const stressTriggersMild = calculateEnvironmentalTriggers(testDate, testLocation, testHorse, mildWeather);

      expect(stressTriggersExtreme.intensity).toBeGreaterThan(stressTriggersMild.intensity);
      expect(stressTriggersExtreme.activeTriggersCount).toBeGreaterThan(stressTriggersMild.activeTriggersCount);
    });
  ...
  describe('🎯 Environmental Impact Calculation', () => {
    ...
    test('should provide different impacts for different environmental conditions', () => {
      ...
      const harshImpact = calculateEnvironmentalImpact(testDate, testLocation, testHorse, harshConditions);
  ...
  describe('📊 Comfort Zone Analysis', () => {
    test('should calculate horse comfort zone based on traits and preferences', () => {
      ...
      expect(comfortZone.temperatureRange.optimal).toBeGreaterThanOrEqual(comfortZone.temperatureRange.min);
      expect(comfortZone.temperatureRange.optimal).toBeLessThanOrEqual(comfortZone.temperatureRange.max);
