...
describe('Enhanced Care Pattern Analyzer', () => {
  ...
  beforeAll(async () => {
    ...
    testUser = await prisma.user.create({
      data: {
        username: `carepattern_${randomBytes(4).toString('hex')}_${randomBytes(4).toString('hex')}`,
        email: `carepattern_${randomBytes(4).toString('hex')}_${randomBytes(4).toString('hex')}@test.com`,
        password: 'test_hash',
        firstName: 'Test',
        lastName: 'User',
        money: 1000,
