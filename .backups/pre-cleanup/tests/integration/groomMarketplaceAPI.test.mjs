...
describe('🏪 INTEGRATION: Groom Marketplace API', () => {
  ...
  describe('POST /api/groom-marketplace/hire', () => {
    ...
    it('should remove hired groom from marketplace', async () => {
      ...
      const hiredGroomStillThere = updatedGrooms.find(
        (g) => g.marketplaceId === groomToHire.marketplaceId
      );
    ...
    it('should reject hiring with insufficient funds', async () => {
      ...
      const expensiveGroom = marketplaceResponse.body.data.grooms.find(
        (g) => g.sessionRate * 7 > 10
      ); // Find groom that costs more than user has
