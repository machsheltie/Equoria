...
describe('🏠 INTEGRATION: Dashboard API - Real Database Integration', () => {
  ...
  beforeEach(async () => {
    ...
    // Generate auth token
    testToken = jwt.sign({ id: testUser.id, email: testUser.email }, config.jwtSecret, {
      expiresIn: '1h',
    });
  ...
  describe('GET /api/user/dashboard/:userId', () => {
    ...
    it('should handle user with no horses gracefully', async () => {
      ...
      const emptyToken = jwt.sign({ id: emptyUser.id, email: emptyUser.email }, config.jwtSecret, {
        expiresIn: '1h',
      });
    ...
    it('should handle user with horses but no activity gracefully', async () => {
      ...
      const inactiveToken = jwt.sign(
        { id: inactiveUser.id, email: inactiveUser.email },
        config.jwtSecret,
        {
          expiresIn: '1h',
        }
