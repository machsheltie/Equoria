...
describe('Enhanced Reporting API Routes', () => {
  ...
  beforeAll(async () => {
    ...
    // Generate auth token
    authToken = jwt.sign({ id: testUser.id, username: testUser.username }, process.env.JWT_SECRET, {
      expiresIn: '1h',
    });
    ...
    for (const horse of testHorses) {
      const ageInDays = Math.floor(
        (now.getTime() - horse.dateOfBirth.getTime()) / (1000 * 60 * 60 * 24)
      );
  ...
  describe('Multi-Horse Analysis Endpoints', () => {
    ...
    test('GET /api/horses/trait-trends should return trait development trends', async () => {
      const response = await request(app)
        ...
        .query({ user: { connect: { id: testUser.id } }, timeframe: 30 })
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200);
