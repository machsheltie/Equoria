...
describe('Breeding Analytics Service', () => {
  ...
  describe('Lineage Tracking', () => {
    ...
    test('should identify parent-offspring relationships', async () => {
      ...
      const pair = analytics.breedingPairs.find(
        (p) => p.stallion.id === testStallion.id && p.mare.id === testMare.id
      );
  ...
  describe('Error Handling', () => {
    ...
    test('should handle non-existent user gracefully', async () => {
      await expect(
        breedingAnalyticsService.getBreedingAnalytics('non-existent-user-id')
