...
describe('Performance Analytics Service', () => {
    ...
    for (let i = 0; i < 20; i++) {
      const result = await prisma.competitionResult.create({
        data: {
          ...
          prizeWon:
            placements[i % placements.length] <= 3
              ? 1000 - placements[i % placements.length] * 200
              : 0,
        },
      });
  ...
  describe('Error Handling', () => {
    test('should handle non-existent horse gracefully', async () => {
      await expect(performanceAnalyticsService.getPerformanceAnalytics(99999)).rejects.toThrow(
        'Horse not found'
      );
    });
