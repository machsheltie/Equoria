...
describe('Dynamic Compatibility Scoring', () => {
  ...
  describe('calculateDynamicCompatibility', () => {
    test('should calculate high compatibility for calm groom with fearful horse', async () => {
      ...
      const compatibility = await calculateDynamicCompatibility(
        calmGroom.id,
        fearfulHorse.id,
        context
      );
    ...
    test('should apply experience bonuses correctly', async () => {
      ...
      const expertCompatibility = await calculateDynamicCompatibility(
        expertGroom.id,
        horse.id,
        context
      );
      const noviceCompatibility = await calculateDynamicCompatibility(
        noviceGroom.id,
        horse.id,
        context
      );

      expect(expertCompatibility.experienceBonus).toBeGreaterThan(
        noviceCompatibility.experienceBonus
      );
  ...
  describe('predictInteractionOutcome', () => {
    ...
    test('should predict negative outcome for poor compatibility', async () => {
      ...
      const prediction = await predictInteractionOutcome(
        energeticGroom.id,
        fearfulHorse.id,
        context
      );
