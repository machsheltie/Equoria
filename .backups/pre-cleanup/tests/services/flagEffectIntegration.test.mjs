...
describe('Flag Effect Integration', () => {
  ...
  describe('applyFlagEffectsToBreeding', () => {
    test('should apply flag effects to breeding trait probabilities', async () => {
      ...
      expect(result.modifiedTraitProbabilities.brave).toBeGreaterThan(
        breedingOutcome.baseTraitProbabilities.brave
      );

      // Should decrease probability of negative traits
      expect(result.modifiedTraitProbabilities.fearful).toBeLessThan(
    ...
    test('should handle breeding between horses with conflicting flags', async () => {
      ...
      const totalNegativeChange =
        ...
        breedingOutcome.baseTraitProbabilities.fearful +
        (result.modifiedTraitProbabilities.insecure -
          breedingOutcome.baseTraitProbabilities.insecure);
  ...
  describe('resolveFlagConflicts', () => {
    test('should identify and resolve flag conflicts', async () => {
      ...
      expect(conflicts.some((c) => c.flags.includes('brave') && c.flags.includes('fearful'))).toBe(
        true
      );
      expect(
        conflicts.some((c) => c.flags.includes('confident') && c.flags.includes('insecure'))
      ).toBe(true);
    });
