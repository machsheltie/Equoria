...
describe('Developmental Window System', () => {
  ...
  describe('identifyDevelopmentalWindows', () => {
    ...
    test('should identify multiple active windows for appropriate ages', async () => {
      ...
      const socializationWindow = windows.activeWindows.find(
        (w) => w.name === 'early_socialization'
      );
  ...
  describe('calculateWindowSensitivity', () => {
    ...
    test('should apply environmental modifiers correctly', async () => {
      ...
      const stressedSensitivity = await calculateWindowSensitivity(
        stressedFoal.id,
        'fear_period_1'
      );
      ...
      expect(stressedSensitivity.environmentalModifier).toBeGreaterThan(
        calmSensitivity.environmentalModifier
      );
    });
  });
  ...
  describe('evaluateTraitDevelopmentOpportunity', () => {
    ...
    test('should show higher opportunities for window-aligned traits', async () => {
      ...
      const braveOpportunity = await evaluateTraitDevelopmentOpportunity(
        fearPeriodFoal.id,
        'brave',
        'fear_period_1'
      );
    ...
    test('should provide appropriate recommendations', async () => {
      ...
      expect(opportunity.recommendedActions.some((action) => action.includes('exploration'))).toBe(
        true
      );
    });
  });
