...
describe('Groom Talent Service', () => {
  ...
  describe('Talent Selection', () => {
    ...
    test('should reject invalid talent selection', async () => {
      await expect(selectTalent(testGroom.id, 'tier1', 'invalid_talent')).rejects.toThrow(
        'Invalid talent selection'
      );
    });
    ...
    test('should reject selection for insufficient level', async () => {
      ...
      await expect(selectTalent(lowLevelGroom.id, 'tier1', 'gentle_hands')).rejects.toThrow(
        'insufficient_level'
      );
