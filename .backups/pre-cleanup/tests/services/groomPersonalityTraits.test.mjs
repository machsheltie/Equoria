...
describe('Groom Personality Trait System', () => {
  ...
  describe('calculatePersonalityModifiers', () => {
    test('should calculate modifiers for calm groom with nervous horse', async () => {
      ...
      const modifiers = await calculatePersonalityModifiers(
        calmGroom.id,
        nervousHorse.id,
        'trust_building'
      );
  ...
  describe('analyzePersonalityCompatibility', () => {
    test('should analyze compatibility between groom and horse personalities', async () => {
      ...
      expect(compatibility.strengths.some((s) => s.includes('calm') || s.includes('patient'))).toBe(
        true
      );
    });
    ...
    test('should identify poor compatibility combinations', async () => {
      ...
      const compatibility = await analyzePersonalityCompatibility(
        energeticGroom.id,
        nervousHorse.id
      );
      ...
      expect(
        compatibility.recommendations.some(
          (r) => r.includes('calm') || r.includes('gentle') || r.includes('avoid')
        )
      ).toBe(true);
    });
  });
