...
beforeAll(async () => {
  await jest.unstable_mockModule(prismaPath, () => ({
    default: {
      ...
      groomAssignment: {
        ...
        findUnique: jest.fn(
          async () => globalThis.ownershipTestMockState.groomAssignmentFindUnique
        ),
        findMany: jest.fn(async () => globalThis.ownershipTestMockState.groomAssignmentFindMany),
      },
      competitionResult: {
        findFirst: jest.fn(
          async () => globalThis.ownershipTestMockState.competitionResultFindFirst
        ),
        findUnique: jest.fn(
          async () => globalThis.ownershipTestMockState.competitionResultFindUnique
        ),
  ...
  mockCompetitionResultFindFirst.mockImplementation(
    async () => mockState.competitionResultFindFirst
  );
  mockCompetitionResultFindUnique.mockImplementation(
    async () => mockState.competitionResultFindUnique
  );
...
describe('Ownership Validation Unit Tests', () => {
  ...
  describe('validateBatchOwnership Helper Function', () => {
    it('should validate ownership of multiple horses using userId', async () => {
      const horses = [
        createMockHorse({ id: 1, userId: 'user-1' }),
        createMockHorse({ id: 2, userId: 'user-1' }),
      ];
