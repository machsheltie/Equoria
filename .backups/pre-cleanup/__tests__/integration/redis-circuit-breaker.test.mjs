...
} from '../../utils/redisCircuitBreaker.mjs';
...
describe('Redis Circuit Breaker Integration Tests', () => {
  ...
  describe('Redis Failure Scenarios', () => {
    ...
    it.skip('should handle intermittent Redis failures', async () => {
      ...
      await expect(circuitBreaker.operations.get.fire('key2')).rejects.toThrow(
      ...
      await expect(circuitBreaker.operations.get.fire('key4')).rejects.toThrow(
        'Intermittent failure'
      );
      await new Promise((resolve) => setTimeout(resolve, 50));
    ...
    it.skip('should handle multiple operation types failing simultaneously', async () => {
      ...
      await expect(circuitBreaker.operations.set.fire('key', 'value')).rejects.toThrow(
        'Set failed'
      );
      await new Promise((resolve) => setTimeout(resolve, 50));
  ...
  describe('Metrics and Health Monitoring', () => {
    ...
    it.skip('should calculate error rate correctly', async () => {
      ...
      expect(healthStatus.metrics.successCount + healthStatus.metrics.failureCount).toBeGreaterThan(
        0
      );
