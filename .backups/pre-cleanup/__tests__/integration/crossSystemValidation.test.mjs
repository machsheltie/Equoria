...
describe('Cross-System Validation Tests', () => {
  ...
  describe('Competition System Integration', () => {
    test('Training system integration with competition performance', async () => {
      ...
      const initialRacingScore =
        (initialStats.disciplineScores && initialStats.disciplineScores.Racing) || 0;
      const updatedRacingScore =
        (updatedStats.disciplineScores && updatedStats.disciplineScores.Racing) || 0;
  ...
  describe('Performance System Integration', () => {
    test('Memory management integration with API operations', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        operations.push(
          request(app).get('/api/horses').set('Authorization', `Bearer ${authToken}`)
        );
      }
