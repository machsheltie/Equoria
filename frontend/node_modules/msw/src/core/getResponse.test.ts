...
it('returns undefined given empty headers array', async () => {
  await expect(getResponse([], new Request('http://localhost/'))).resolves.toBeUndefined();
});

it('returns undefined given no matching handlers', async () => {
  await expect(
    getResponse([http.get('/product', () => void 0)], new Request('http://localhost/user'))
  ).resolves.toBeUndefined();
});

it('returns undefined given a matching handler that returned no response', async () => {
  await expect(
    getResponse([http.get('*/user', () => void 0)], new Request('http://localhost/user'))
  ).resolves.toBeUndefined();
});

it('returns undefined given a matching handler that returned explicit undefined', async () => {
  await expect(
    getResponse([http.get('*/user', () => undefined)], new Request('http://localhost/user'))
  ).resolves.toBeUndefined();
});
