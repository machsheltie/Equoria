...
const resolver: ResponseResolver<GraphQLResolverExtras<{ userId: string }>> = ({ variables }) => {
...
describe('info', () => {
  it('exposes request handler information for query', () => {
    const handler = new GraphQLHandler(OperationTypeNode.QUERY, 'GetUser', '*', resolver);
  ...
  it('exposes request handler information for mutation', () => {
    const handler = new GraphQLHandler(OperationTypeNode.MUTATION, 'Login', '*', resolver);
  ...
  it('parses a query operation name from a given DocumentNode', () => {
    ...
    const handler = new GraphQLHandler(OperationTypeNode.QUERY, node, '*', resolver);
  ...
  it('parses a mutation operation name from a given DocumentNode', () => {
    ...
    `);
    const handler = new GraphQLHandler(OperationTypeNode.MUTATION, node, '*', resolver);
  ...
  it('throws an exception given a DocumentNode with a mismatched operation type', () => {
    ...
    expect(() => new GraphQLHandler(OperationTypeNode.QUERY, node, '*', resolver)).toThrow(
...
describe('parse', () => {
  ...
  describe('mutation', () => {
    it('parses a mutation without variables (GET)', async () => {
      const handler = new GraphQLHandler(OperationTypeNode.MUTATION, 'GetUser', '*', resolver);
      const request = createGetGraphQLRequest({
        query: LOGIN,
      });
...
describe('request', () => {
  it('has parsed operationName', async () => {
    ...
    const handler = new GraphQLHandler(OperationTypeNode.QUERY, /.*/, '*', matchAllResolver);
    const request = createPostGraphQLRequest({
      query: `
        query GetAllUsers {
          user {
    ...
    expect(matchAllResolver.mock.calls[0][0]).toHaveProperty('operationName', 'GetAllUsers');
  });
});
