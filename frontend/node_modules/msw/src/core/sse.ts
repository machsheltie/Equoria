...
export type ServerSentEventMessage<EventMap extends EventMapConstraint = { message: unknown }> =
...
class ServerSentEventHandler<EventMap extends EventMapConstraint> extends HttpHandler {
  ...
  #attachClientLogger(request: Request, emitter: Emitter<ServerSentEventClientEventMap>): void {
    const publicUrl = toPublicUrl(request.url);

    /* eslint-disable no-console */
    emitter.on('message', (payload) => {
      console.groupCollapsed(
        devUtils.formatMessage(`${getTimestamp()} SSE %s %c⇣%c ${payload.event}`),
        publicUrl,
        `color:${colors.mocked}`,
        'color:inherit'
      );
...
class ServerSentEventClient<EventMap extends EventMapConstraint = { message: unknown }> {
  #encoder: TextEncoder;
  ...
  #sendMessage(message: { id?: string; event?: unknown; data: unknown | undefined }): void {
...
type EventHandler<EventType extends Event> = (this: EventSource, event: EventType) => any;
...
class EventSourceParsingStream extends WritableStream {
  ...
  private processLine(line: Uint8Array, fieldLength: number): void {
    ...
    if (fieldLength > 0) {
      ...
      const valueOffset = fieldLength + (line[fieldLength + 1] === ControlCharacters.Space ? 2 : 1);
