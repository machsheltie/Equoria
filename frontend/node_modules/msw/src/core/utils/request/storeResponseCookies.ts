...
export async function storeResponseCookies(request: Request, response: Response): Promise<void> {
  // Grab the raw "Set-Cookie" response header provided
  // in the HeadersInit for this mocked response.
  const responseCookies = Reflect.get(response, kSetCookie) as string | undefined;
