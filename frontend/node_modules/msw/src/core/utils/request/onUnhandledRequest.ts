...
export type UnhandledRequestCallback = (request: Request, print: UnhandledRequestPrint) => void;

export type UnhandledRequestStrategy = 'bypass' | 'warn' | 'error' | UnhandledRequestCallback;
...
export async function onUnhandledRequest(
  ...
  const requestBody =
    request.method === 'HEAD' || request.method === 'GET' ? null : await request.clone().text();
