...
test('supports the "bypass" request strategy', async () => {
  await onUnhandledRequest(new Request(new URL('http://localhost/api')), 'bypass');
...
test('supports a custom callback function', async () => {
  ...
  expect(console.warn).toHaveBeenCalledWith(`callback: GET http://localhost:3000/user`);
});
...
test('prints with a relative URL and search params', async () => {
  await onUnhandledRequest(new Request(new URL('http://localhost/api?foo=boo')), 'warn');

  expect(console.warn).toHaveBeenCalledWith(fixtures.warningWithoutSuggestions(`/api?foo=boo`));
});

test('prints with an absolute URL and search params', async () => {
  await onUnhandledRequest(new Request(new URL('https://mswjs.io/api?foo=boo')), 'warn');
...
test('ignores common static assets when using the "warn" strategy', async () => {
  await Promise.allSettled([
    onUnhandledRequest(new Request(new URL('https://example.com/main.css')), 'warn'),
    onUnhandledRequest(new Request(new URL('https://example.com/index.mjs')), 'warn'),
    onUnhandledRequest(new Request(new URL('https://example.com/node_modules/abc-123')), 'warn'),
    onUnhandledRequest(new Request(new URL('https://fonts.googleapis.com/some-font')), 'warn'),
  ]);
...
test('ignores common static assets when using the "error" strategy', async () => {
  await Promise.allSettled([
    onUnhandledRequest(new Request(new URL('https://example.com/main.css')), 'error'),
    onUnhandledRequest(new Request(new URL('https://example.com/index.mjs')), 'error'),
    onUnhandledRequest(new Request(new URL('https://example.com/node_modules/abc-123')), 'error'),
    onUnhandledRequest(new Request(new URL('https://fonts.googleapis.com/some-font')), 'error'),
  ]);
...
test('exposes common static assets to the explicit callback', async () => {
  ...
  await onUnhandledRequest(new Request(new URL('https://example.com/main.css')), (request) => {
