...
describe('matchRequestUrl', () => {
  test('returns true when matches against an exact URL', () => {
    const match = matchRequestUrl(new URL('https://test.mswjs.io'), 'https://test.mswjs.io');
  ...
  test('returns true when matched against a RegExp', () => {
    const match = matchRequestUrl(new URL('https://test.mswjs.io'), /test\.mswjs\.io/);
    expect(match).toHaveProperty('matches', true);
    expect(match).toHaveProperty('params', {});
  });
  ...
  test('returns false when does not match against the request URL', () => {
    const match = matchRequestUrl(new URL('https://test.mswjs.io'), 'https://google.com');
  ...
  test('returns true for matching WebSocket URL', () => {
    expect(matchRequestUrl(new URL('ws://test.mswjs.io'), 'ws://test.mswjs.io')).toEqual({
      matches: true,
      params: {},
    });
    expect(matchRequestUrl(new URL('wss://test.mswjs.io'), 'wss://test.mswjs.io')).toEqual({
      matches: true,
      params: {},
    });
  });
  ...
  test('returns false for non-matching WebSocket URL', () => {
    expect(matchRequestUrl(new URL('ws://test.mswjs.io'), 'ws://foo.mswjs.io')).toEqual({
      matches: false,
      params: {},
    });
    expect(matchRequestUrl(new URL('wss://test.mswjs.io'), 'wss://completely.diff')).toEqual({
      matches: false,
      params: {},
    });
  });
  ...
  test('returns path parameters when matched a WebSocket URL', () => {
    expect(matchRequestUrl(new URL('wss://test.mswjs.io'), 'wss://:service.mswjs.io')).toEqual({
      matches: true,
      params: {
        service: 'test',
      },
...
describe('coercePath', () => {
  test('escapes the colon in protocol', () => {
    ...
    expect(coercePath('https://example.com/:userId')).toEqual('https\\://example.com/:userId');
    expect(coercePath('http://localhost:3000')).toEqual('http\\://localhost\\:3000');
  });

  test('escapes the colon before the port number', () => {
    expect(coercePath('localhost:8080')).toEqual('localhost\\:8080');
    expect(coercePath('http://127.0.0.1:8080')).toEqual('http\\://127.0.0.1\\:8080');
    expect(coercePath('https://example.com:1234')).toEqual('https\\://example.com\\:1234');
  ...
  test('replaces wildcard with an unnnamed capturing group', () => {
    ...
    expect(coercePath('https://example.com/user/*')).toEqual('https\\://example.com/user/(.*)');
    expect(coercePath('https://example.com/us*')).toEqual('https\\://example.com/us(.*)');
  });
  ...
  test('preserves path parameter modifiers', () => {
    ...
    expect(coercePath('/foo/**:name*')).toEqual('/foo/(.*):name*');
    expect(coercePath('**/foo/*/:name*')).toEqual('(.*)/foo/(.*)/:name*');
    expect(coercePath('/foo/:first/bar/:second*/*')).toEqual('/foo/:first/bar/:second*/(.*)');
  });
});
