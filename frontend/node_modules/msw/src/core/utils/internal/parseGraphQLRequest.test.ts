...
import { ParsedGraphQLRequest, parseGraphQLRequest } from './parseGraphQLRequest';

test('returns true given a GraphQL-compatible request', async () => {
  const getRequest = new Request(
    new URL('http://localhost:8080/graphql?query=mutation Login { user { id } }')
  );
  expect(await parseGraphQLRequest(getRequest)).toEqual<ParsedGraphQLRequest<any>>({
  ...
  expect(await parseGraphQLRequest(postRequest)).toEqual<ParsedGraphQLRequest<any>>({
