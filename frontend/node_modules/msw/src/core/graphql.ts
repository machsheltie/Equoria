...
import { ResponseResolver, RequestHandlerOptions } from './handlers/RequestHandler';
import {
  GraphQLHandler,
  GraphQLVariables,
...
>(
  predicate:
    ...
    | GraphQLCustomPredicate,
  resolver: GraphQLResponseResolver<[Query] extends [never] ? GraphQLQuery : Query, Variables>,
...
function createGraphQLOperationHandler(url: Path) {
  return <
    ...
    Variables extends GraphQLVariables = GraphQLVariables,
  >(
    resolver: ResponseResolver<GraphQLResolverExtras<Variables>, null, GraphQLResponseBody<Query>>
  ) => {
