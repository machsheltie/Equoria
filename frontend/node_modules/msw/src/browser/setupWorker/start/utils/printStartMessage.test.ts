...
test('prints out a default start message into console', () => {
  ...
  expect(console.log).toHaveBeenCalledWith('Found an issue? https://github.com/mswjs/msw/issues');

  // Includes service worker scope.
  expect(console.log).toHaveBeenCalledWith('Worker scope:', 'http://localhost:3000/');

  // Includes service worker script location.
  expect(console.log).toHaveBeenCalledWith('Worker script URL:', 'http://localhost:3000/worker.js');
});
...
test('prints a worker scope in the start message', () => {
  ...
  expect(console.log).toHaveBeenCalledWith('Worker scope:', 'http://localhost:3000/user');
});
