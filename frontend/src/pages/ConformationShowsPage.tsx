/**
 * ConformationShowsPage (Equoria-e0cn)
 *
 * Lists the open conformation shows the user can enter horses into. Reuses
 * `useCompetitions` (returns all open shows including ridden + conformation)
 * and filters by `showType === 'conformation'`. Each row offers an "Enter"
 * action that opens `ConformationEntryModal` against the chosen horse.
 *
 * This page is intentionally separate from CompetitionBrowserPage to keep
 * the existing browser surface stable while the conformation system stabilizes.
 * Wiring into the unified browser as a tab is a follow-up (out of scope here).
 */
...
const ConformationShowsPage = (): JSX.Element => {
  ...
  return (
    <div className="container mx-auto px-4 py-6" data-testid="conformation-shows-page">
      <PageHero title="Conformation Shows" icon={<Trophy className="h-8 w-8 text-amber-600" />}>
        <p className="text-sm text-gray-700">
          Compete on the conformation circuit — judges score type, balance, and movement regardless
          of training discipline. Title points earned here add a permanent breeding-value boost to
          your horse.
        </p>
      </PageHero>
