 ...
 * affordance. Built on GameDialog (Equoria-o5hub.13, DECISIONS.md §8) —
 * Radix Dialog provides the focus trap, Escape close, scroll-lock, focus
 * restoration, and backdrop-click dismissal.
 *
...
const LiveTraitDetailModal: React.FC<LiveTraitDetailModalProps> = ({ isOpen, onClose, trait }) => {
  ...
  return (
    ...
    >
      {/* No GameDialogDescription: the trait's Description section is the body
          content itself; aria-describedby={undefined} suppresses the Radix
          missing-description warning without fabricating a second summary. */}
      <GameDialogContent size="md" data-testid="trait-detail-modal" aria-describedby={undefined}>
        <GameDialogHeader>
          <GameDialogTitle>{trait.name}</GameDialogTitle>
        </GameDialogHeader>

        <GameDialogBody>
          <div className="space-y-4 pt-2" data-testid="trait-detail-content">
