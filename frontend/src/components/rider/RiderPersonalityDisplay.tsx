              ...
              className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${
                affinity.magnitude === 'high'
                  ...
                  : affinity.magnitude === 'medium'
                    ...
                    : 'bg-[var(--glass-surface-subtle-bg)] text-[var(--text-muted)]'
              }`}
            >
              {affinity.discipline}
            </span>
        ...
        <div className="space-y-1.5">
          {info.disciplineAffinities.map((affinity) => (
            <div key={affinity.discipline} className="flex items-center justify-between text-sm">
              <span className="text-[var(--text-secondary)]">{affinity.discipline}</span>
              <span
                className={
                  affinity.magnitude === 'high'
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {info.temperamentCompatibility.map((compat) => (
            ...
            >
              <span className="text-[var(--text-secondary)]">{compat.horseTemperament}</span>
